# AGENTS.md — Project Development Contract
...
Read AGENTS.md
↓
Load .Codex/skills/animejs/SKILL.md (Skill tool)
↓
...
|---|---|
| `.Codex/skills/<name>/` | Registered with Codex — invoke via the **Skill tool** |
| `.agents/skills/<name>/` | **Not** registered — load by **reading `SKILL.md` as a file** |

There are **21 skills**: 20 in `.agents/skills/`, 1 in `.Codex/skills/`.
...
**Path:** [.Codex/skills/animejs/](.Codex/skills/animejs/) — load via the **Skill tool** (`animejs`)
...
Identify required skills          ← §13 Skill Selection Matrix
     ↓
Load required SKILL.md files      ← Skill tool for .Codex/, file read for .agents/
     ↓
...
Run validation                    ← §15
     ↓
Review against AGENTS.md          ← §16 Definition of Done
     ↓
Finish
```
...
Identify: animation task (existing component, no new visual direction)
     ↓
Load .Codex/skills/animejs/SKILL.md via the Skill tool
     ↓
...
**When a new skill is added** to `.agents/skills/` or `.Codex/skills/`, add a
...
**Current count: 21 skills — 20 in `.agents/skills/`, 1 in `.Codex/skills/`.**
